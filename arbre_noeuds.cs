...
{
    public class arbre_noeuds : noeud_proprietes
    {
        #region
        /// <summary>
        ...
        public noeud_proprietes Tree;
        public noeud_proprietes[] tempNode;
        private byte[] toNode;
        public byte[] TheResult;
        public arbre_noeuds.huffword[] TheCarWord = new huffword[256];
        ...
        public arbre_noeuds(string filePath)
        {
        ...
        public arbre_noeuds(string path, int b, ref int taille)
        {
            //CompressWrite
        ...
        public arbre_noeuds(Stream sin)
        {
            toNode = new byte[sin.Length];
            sin.Read(toNode, 0, (int)sin.Length);
            sin.Close();
        ...
        public noeud_proprietes[] getFreqKey()
        {
            int i = 0;
            int NodeCount = 0;
            ...
            noeud_proprietes[] Feuille = new noeud_proprietes[NodeCount];
            ...
            for (i = 0; i < NodeCount; i++)
            {
                for (int j = Debut; j <= 255; j++)
                {
                    ...
                    {
                        Debut = j + 1;
                        Feuille[i] = new noeud_proprietes(j, CarCount[j]);
                        break;
                    }
                }
            }
        ...
        public noeud_proprietes CreateTree(ref noeud_proprietes[] cNode)
        {
            noeud_proprietes[] tampon = new noeud_proprietes[(2 * cNode.Length) - 1];
            long min1 = 0;
            long min2 = 0;
            int IdLNode = -1;
            int IdRNode = -1;
            ...
            for (int i = NodeCount; i >= 2; i--)
            {
                ...
                tampon[NodeCount] = new noeud_proprietes(ref tampon[IdLNode], ref tampon[IdRNode]);
        ...
        public void CreateHuffSeq(ref noeud_proprietes curNode, ref arbre_noeuds.huffword[] CarWord, ref short nbNoeud)
        {
        ...
        {
            noeud_proprietes[] tampon = new noeud_proprietes[tempNode.Length];
            int curTampon = 0;
            for (int i = 0; i <= 255; i++)
            {
        ...
        public byte[] Decompress()
        {
            ...
            if (decomp)
            {
                ...
                noeud_proprietes[] cNode = new noeud_proprietes[nbNoeud];
                curByte = 11;
                int BitPos = 0;

                for (int i = 0; i < nbNoeud; i++)
                {
                    cNode[i] = new noeud_proprietes();
                ...
                Tree = new noeud_proprietes(-1, 0);
                Tree.parent = null;
                noeud_proprietes curNode = Tree;
                int ResultLen = 0;
                ...
                for (int i = 0; i < cNode.Length; i++)
                {
                    ...
                    for (int j = 0; j < cNode[i].CodeWord.Length; j++)
                    {
                        if (cNode[i].CodeWord.Substring(j, 1).Equals("0") && j < cNode[i].CodeWord.Length - 1)
                        {
                            ...
                            {
                                curNode.LNode = new noeud_proprietes(-1, 0);
                        ...
                        else if (cNode[i].CodeWord.Substring(j, 1).Equals("1") && j < cNode[i].CodeWord.Length - 1)
                        {
                            ...
                            {
                                curNode.RNode = new noeud_proprietes(-1, 0);
        ...
        public byte[] Compress(Stream sin)
        {
            ...
            Tree = CreateTree(ref tempNode);
            tempNode = new noeud_proprietes[nbNoeud];
